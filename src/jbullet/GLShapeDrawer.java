...
import static jbullet.IGL.GL_COLOR_MATERIAL;
import static jbullet.IGL.GL_LINES;
import static jbullet.IGL.GL_TRIANGLES;
...
public class GLShapeDrawer {
	...
	public static void drawOpenGL(IGL gl, Transform trans,
			CollisionShape shape, Vector3f color, int debugMode) {
		...
		if (shape.getShapeType() == BroadphaseNativeType.COMPOUND_SHAPE_PROXYTYPE) {
			...
			if ((debugMode & DebugDrawModes.DRAW_WIREFRAME) == 0) {
				...
				case BOX_SHAPE_PROXYTYPE: {
					...
					Vector3f halfExtent = boxShape
							.getHalfExtentsWithMargin(vectorsPool.get());
					gl.glScalef(2f * halfExtent.x, 2f * halfExtent.y,
							2f * halfExtent.z);
					gl.drawCube(1f);
				...
				case SPHERE_SHAPE_PROXYTYPE: {
					...
					/*
					 * glPointSize(10f); glBegin(GL_POINTS); glVertex3f(0f, 0f,
					 * 0f); glEnd(); glPointSize(1f);
					 */
					useWireframeFallback = false;
					break;
				}
				...
				case STATIC_PLANE_PROXYTYPE: {
					...
					Vector3f planeNormal = staticPlaneShape
							.getPlaneNormal(vectorsPool.get());
				...
				case CYLINDER_SHAPE_PROXYTYPE: {
					...
					float halfHeight = VectorUtil.getCoord(
							cylinder.getHalfExtentsWithMargin(halfVec), upAxis);
			...
			// {
			// GLuint dlist = OGL_get_displaylist_for_shape((btCollisionShape *
			// )shape);
			// if (dlist)
			// {
			// glCallList(dlist);
			...
			// if (shape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE)
			{
				...
				// btVector3
				// aabbMax(btScalar(1e30),btScalar(1e30),btScalar(1e30));
				// btVector3
				// aabbMax(100,100,100);//btScalar(1e30),btScalar(1e30),btScalar(1e30));
			...
			// btVector3
			// aabbMin(-btScalar(1e30),-btScalar(1e30),-btScalar(1e30));
			// TriangleGlDrawcallback drawCallback;
			// convexMesh->getMeshInterface()->InternalProcessAllTriangles(&drawCallback,aabbMin,aabbMax);
			//
			// }

			// TODO: error in original sources GL_DEPTH_BUFFER_BIT instead of
			// GL_DEPTH_TEST
	...
	private static class GlDisplaylistDrawcallback extends TriangleCallback {
		...
		public void processTriangle(Vector3f[] triangle, int partId,
				int triangleIndex) {
			 ...
			 * triangle[1].getZ()); glColor3f(1, 1, 0);
			 * glNormal3d(normal.getX(),normal.getY(),normal.getZ());
	...
	private static class TriangleGlDrawcallback extends
		...
		public void internalProcessTriangleIndex(Vector3f[] triangle,
